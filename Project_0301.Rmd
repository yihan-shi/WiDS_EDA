...
```{r create test set}
set.seed(0219)
indices <- sample(nrow(training_1), 7675)
```
...
```{r knn}
mod_knn <-  knn(train, test, train_status, 
               k = 10, prob = F, use.all = F)
mod_knn == true_status
mean(mod_knn == true_status)
```
95.2% of the test data was accurately classified.

```{r}
predictions <- mod_knn
predictions_df <- data.frame(id = 1:length(mod_knn), 
                             prediction = predictions,
                             stringsAsFactors = FALSE)
write.csv(predictions_df, "predictions.csv") 
getwd()
```
